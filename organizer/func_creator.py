...
for j in range(0, 30):
    ...
    home_team = open('teams.txt', 'r').readline(j)

    class_def_file = open(f"functions_{home_team}.txt", "w")
    class_def_file.close()

    for i in range(0,30):
        
        dictionary_in_use = dics[j]
        team = open('teams.txt', "r").readline(i)

        away_team_url = dictionary_in_use[f"{team}"]
        ...
        new_team_func = team_func(team, away_team_url)
        team_def_file = open(f'functions_{home_team}.txt', 'a')
        team_def_file.write(new_team_func)
        team_def_file.write('\n')
        team_def_file.close
